...
def compute_hi(q):
    ...
    print("теоретические вероятности попадания нормально распределенной случайной величины в частичный интервал",theoretical_probabilities)
    ...
    # числитель (n_j - N*p)^2
    # данные гистограммы
    hist_values, _ = np.histogram(array_data, bins=q)
    numerator = []
    for i in range(q):
